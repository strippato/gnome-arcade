...
rom_loadItemAsync (struct rom_romItem* item)
{
    ...
    if (g_file_test (fileNamePng, G_FILE_TEST_EXISTS)) {
        ...
        g_file_read_async (item->tileFile, G_PRIORITY_HIGH_IDLE, FALSE, (GAsyncReadyCallback) rom_fileRead_cb, item);
        ...
        g_file_read_async (item->tileFile, G_PRIORITY_HIGH_IDLE, FALSE, (GAsyncReadyCallback) rom_fileReadNoScale_cb, item);
    } else {
        if (www_autoDownload) {
            www_download (item);
        } else {
